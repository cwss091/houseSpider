...
table_title = [u"小区", u"总价", u"单价", u"建筑面积", u"套内面积", u"年代", u"行政区", u"街道", u"环位", u"所在楼层", u"房屋户型", u"房屋朝向", u"梯户比例", u"户型结构", u"装修情况", u"配备电梯", u"建筑类型", u"产权年限"]
		...
		with open(file, 'r') as f:
			...
			for row in freader:
				...
				if len(row) < 17:
					...
					continue
				community = row[8]
				total_price = row[1].split(u" ")[0]
				unit_price = row[12]
				space_area = row[13]
				actual_space_area = row[7]
				year = row[14]
				#area = row[3].split(" ".encode("utf-8"))
				#print type(row[3])
				#print repr(row[3])
				area = row[3].split(u'\xa0')
				...
				room_face = row[4]
				unit_lift = row[0]
				room_structure = row[6]
				decor = row[10]
				islift = row[11]
				building = row[16]
				ownership = row[2]
				info = [community, total_price, unit_price, space_area, actual_space_area, year, dist, street, ring, floor, room, room_face, unit_lift, room_structure, decor, islift, building, ownership]
